...
type MediaUpload = DelayedValue<NodeAttributes>;
    ...
    return {
      ...
      uploadMedia: (
        ...
        return delayedCommand({
          ...
          onFail: (parameter) =>
            this.store.rawCommands.removePlaceholder(value)(parameter),
        });
      },
    };
  }
...
function uploadHandler(files: FileWithProgress[]): MediaUpload[] {
  ...
  let completed = 0;
  const promises: Array<Promise<NodeAttributes>> = [];

  for (const { file, progress } of files) {
    promises.push(
      new Promise<NodeAttributes>((resolve) => {
