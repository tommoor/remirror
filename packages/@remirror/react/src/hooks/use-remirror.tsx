...
export interface UseRemirrorOutput<ExtensionUnion extends AnyExtension> {
  /**
   ...
   */
  getContext: () => ReactFrameworkOutput<ExtensionUnion>;
}
...
export function useRemirror<ExtensionUnion extends AnyExtension>(
...
): UseRemirrorOutput<ReactExtensionUnion<ExtensionUnion>> {
  const { content, document, stringHandler, selection, extensions, ...settings } = props;
  ...
  const getContext = useCallback(() => {
    ...
    invariant(context, { code: ErrorConstant.REACT_GET_CONTEXT });
