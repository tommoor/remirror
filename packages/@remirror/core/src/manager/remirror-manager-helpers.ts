...
export function transformExtensions<RawExtensions extends AnyExtension>(
  ...
  const gatherRawExtensionConfig = { duplicateMap, parentExtensions, rawExtensions, settings };

  for (const extension of initialExtensions) {
    gatherRawExtensions(gatherRawExtensionConfig, { extension });
  }
...
function gatherRawExtensions<ExtensionUnion extends AnyExtension>(
  ...
  const { rawExtensions, duplicateMap, parentExtensions, settings } = config;
 ...
 */
export interface ExtensionTransformation<RawExtensions extends AnyExtension> {
  /**
   ...
   */
  extensions: Array<GetExtensions<RawExtensions>>;
   ...
   */
  extensionMap: WeakMap<GetConstructor<GetExtensions<RawExtensions>>, GetExtensions<RawExtensions>>;
}
