...
export const [DevProvider, useDev] = createContextState<A>(({ set }) => ({}));
...
export function buildColors(schema) {
  return Object.keys(schema.nodes).reduce((acc, node, index) => {
    const safeIndex =
      index >= nodesColors.length ? calculateSafeIndex(index, nodesColors.length) : index;
...
export default class EditorStateContainer extends Container {
  ...
  constructor(editorView, props) {
    ...
    this.diffWorker =
      props && props.diffWorker
        ...
        : import('./json-diff-main').then(({ JsonDiffMain }) => new JsonDiffMain());
    ...
    subscribeOnUpdates(editorView, (tr, oldState, newState) => {
      ...
      this.setState({
        ...
        historyRolledBackTo: updatedHistory ? false : this.state.historyRolledBackTo,
      });
    });
  }
  ...
  updateNodePickerPossition = (target) => {
    ...
    if (
      node &&
      ((node.pmViewDesc.node && node.pmViewDesc.node.type.name !== 'doc') || node.pmViewDesc.mark)
    ) {
  ...
  nodePickerSelect = (target) => {
    ...
    if (node) {
      ...
      const path = findNodeIn(editorState.doc, editorState.doc.nodeAt(node.pmViewDesc.posAtStart));
