...
test('can be configured with attribute node extensions', () => {
  ...
  expect(dom).toContainHTML(prosemirrorNodeToHtml({ node: h3(expected), schema }));
  expect(dom).toContainHTML(prosemirrorNodeToHtml({ node: h2(expected), schema }));
});
...
describe('add', () => {
  ...
  it('overwrites the whole doc on each call', () => {
    ...
    const expected = prosemirrorNodeToHtml({ node, schema });
    const expectedTwo = prosemirrorNodeToHtml({ node: nodeTwo, schema });
  ...
  it('can be chained', () => {
    ...
    expect(dom).toContainHTML(prosemirrorNodeToHtml({ node, schema }));
  });
